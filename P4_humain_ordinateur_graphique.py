...
########################################################################
#
#  Puissance 4 - Humain contre Ordinateur avec IA (mode Graphique)
#
#  Copyright 2016-2019 - Eric Sérandour
#  Version du 06 octobre 2019 à 19 h 48
#
...
def mouse_clic(event):
    ...
    if not blocageJoueur:
        if finPartie:
            ...
            fenetreJeu.update()
            time.sleep(1)
            couleur = joueur_qui_commence('random')
            ...
            while col < NB_COLONNES:
                col = col + 1
                if x > col*ESPACEMENT+2*(col-1)*rayon and x < col*ESPACEMENT+2*col*rayon:
                    colonne = col
            if (colonne and not colonne_pleine(colonne)):
                blocageJoueur = True
                jouer(colonne)
                finPartie = fin_partie()  # Teste si la partie est finie
                if not finPartie :
...
finPartie = False
couleur = joueur_qui_commence('random')  # 'yellow' ou 'red' ou 'random'
affiche_joueur_qui_commence()
if couleur == 'red':
    jouer_ordi_ia(IA_ROUGE)
    finPartie = fin_partie()  # Teste si la partie est finie
